...
class OkapiBM25Ranker:
    ...
    def lookup_query(self, query: str) -> List[Tuple[str, float]]:
        ...
        sorted_results = list(sorted(scores.items(), key=lambda x: x[1], reverse=True))[: OkapiBM25Ranker.N_RESULTS]
        return [(doc_id, self.collection.get_document(doc_id), score) for doc_id, score in sorted_results]
